...
def inpainting_main():
    ...
    args.images = 'inpainting_model/samples/testset' # input image directory
    args.masks = 'inpainting_model/samples/maskset' # input mask director
    args.output_dir = 'inpainting_model/GPU_CPU/results' # output directory
    ...
    with tf.Graph().as_default():
      with open('inpainting_model/GPU_CPU/pb/hifill.pb', "rb") as f:
      ...
      with tf.Session() as sess:
        ...
        for path_img, path_mask in zip(paths_img, paths_mask):
            ...
            cv2.imwrite(filename, inpainted)

    f = open(args.output_dir + '/1.png', 'rb')
    result = f.read()
    f.close()
