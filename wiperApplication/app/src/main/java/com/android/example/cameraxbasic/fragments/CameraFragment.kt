...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        controls.findViewById<ImageButton>(R.id.camera_ratio).setOnClickListener {
            ...
            var top_3_4 = container.findViewById<View>(R.id.top_3_4)
            var top_1_1 = container.findViewById<View>(R.id.top_1_1)
            var bottom_3_4 = container.findViewById<View>(R.id.bottom_3_4)
            var bottom_1_1 = container.findViewById<View>(R.id.bottom_1_1)
            // ratioNum => (0)1:1 (1)3:4 (2)full
            ...
            when (ratioNum) {
                ...
                "0"-> {
                    ratioNum="1"
                    ratioButton.setBackgroundResource(R.drawable.ic_ratio_one_to_one)
                    top_3_4.visibility = View.INVISIBLE
                    bottom_3_4.visibility = View.INVISIBLE
                    top_1_1.visibility = View.VISIBLE
                    bottom_1_1.visibility = View.VISIBLE
                }
                // 3:4
                "1" -> {
                    ratioNum="2"
                    ratioButton.setBackgroundResource(R.drawable.ic_ratio_three_to_four)
                    top_3_4.visibility = View.VISIBLE
                    bottom_3_4.visibility = View.VISIBLE
                    top_1_1.visibility = View.INVISIBLE
                    bottom_1_1.visibility = View.INVISIBLE

                }
                // full
                "2" -> {
                    ratioNum="0"
                    ratioButton.setBackgroundResource(R.drawable.ic_ratio_full)
                    top_3_4.visibility = View.INVISIBLE
                    bottom_3_4.visibility = View.INVISIBLE
                    top_1_1.visibility = View.INVISIBLE
                    bottom_1_1.visibility = View.INVISIBLE
         ...
         */
// Timer
        val txtTimerCount = controls.findViewById<TextView>(R.id.txtTimerCount)
        var timerFlag = 0
        var timerCount = 0
// Timer Handler
        val handler = object : Handler(Looper.getMainLooper()) {
            override fun handleMessage(msg: Message) {
                txtTimerCount.visibility = View.VISIBLE
                txtTimerCount.text = timerCount.toString()
                timerCount--
                if(timerCount == -1) {
                    takePhoto()
                    txtTimerCount.visibility = View.INVISIBLE
                }
            }
        }
        controls.findViewById<ImageButton>(R.id.camera_capture_button).setOnClickListener {
        ...
        val btnTimerOnOff = controls.findViewById<ImageButton>(R.id.btnTimerOnOFF)
        btnTimerOnOff.setOnClickListener {
            when(timerFlag){
                0 -> {
                    timerCount = 3
                    txtTimerCount.visibility = View.VISIBLE
                    txtTimerCount.text = timerCount.toString()
                    btnTimerOnOff.setBackgroundResource(R.drawable.ic_timer3_white)
                    timerFlag = 1
                }
                1 -> {
                    timerCount = 7
                    txtTimerCount.visibility = View.VISIBLE
                    txtTimerCount.text = timerCount.toString()
                    btnTimerOnOff.setBackgroundResource(R.drawable.ic_timer7_white)
                    timerFlag = 2
                }
                2 -> {
                    timerCount = 0
                    txtTimerCount.visibility = View.INVISIBLE
                    txtTimerCount.text = timerCount.toString()
                    btnTimerOnOff.setBackgroundResource(R.drawable.ic_timer_white)
                    timerFlag = 0
                }
            }
        }
    ...
    /**
     * takePhoto 블로그에서 가져온 소스를 여기에 있는 소스로 붙여넣기함 오류가 날수도 있음.
     */
    private fun takePhoto() {
// Get a stable reference of the modifiable image capture use case
//        수정 가능한 이미지 캡처 사용 사례의 안정적인 참조를 가져옵니다.
        val imageCapture = imageCapture ?: return
// Create time-stamped output file to hold the image
//        이미지를 보관할 타임 스탬프 출력 파일 생성
        val photoFile = createFile(outputDirectory, FILENAME, PHOTO_EXTENSION)
        val metadata = Metadata().apply {
            // Mirror image when using the front camera
//            전면 카메라 사용시 미러 이미지
...
// Create output options object which contains file + metadata
//        파일 + 메타 데이터를 포함하는 출력 옵션 개체를 만듭니다.
        val outputOptions = ImageCapture.OutputFileOptions.Builder(photoFile)
            .setMetadata(metadata)
            .build()
        ...
        imageCapture.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                ...
                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    ...
                    Log.d(TAG, "Photo capture succeeded: $savedUri")
                    // We can only change the foreground Drawable using API level 23+ API
//                    API 레벨 23+ API를 사용하여 전경 Drawable 만 변경할 수 있습니다.
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        // Update the gallery thumbnail with latest picture taken
//                        최근에 찍은 사진으로 갤러리 썸네일 업데이트
                    ...
                    // Implicit broadcasts will be ignored for devices running API level >= 24
                    // so if you only target API level 24+ you can remove this statement
//                    API 레벨> = 24를 실행하는 기기에서는 암시 적 브로드 캐스트가 무시되므로 API 레벨 24 이상 만 타겟팅하는 경우이 문을 제거 할 수 있습니다.
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                        requireActivity().sendBroadcast(
                            Intent(android.hardware.Camera.ACTION_NEW_PICTURE, savedUri)
                        )
                    ...
                    // unnecessary but otherwise other apps will not be able to access our
                    // images unless we scan them using [MediaScannerConnection]
//                    선택한 폴더가 외부 미디어 디렉토리 인 경우
//                    불필요하지만 그렇지 않으면 [MediaScannerConnection]을 사용하여 이미지를 스캔하지 않는 한 다른 앱에서 이미지에 액세스 할 수 없습니다.
                    val mimeType = MimeTypeMap.getSingleton()
                        .getMimeTypeFromExtension(savedUri.toFile().extension)
                    MediaScannerConnection.scanFile(
                        context,
